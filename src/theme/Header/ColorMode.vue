...
<template>
    <button type="button" class="color-scheme ml-6 hover:text-slate-500 dark:hover:text-slate-300" @click="toggleDD">
        <IconMoon v-if="isDark" :isActive="buttonState === 'dark'" />
        <IconSun v-else :isActive="buttonState === 'light'" />
    </button>
    <ul v-if="showDD"
        class="absolute z-50 top-full border dark:border-slate-50/[0.06] right-0 bg-white rounded-lg ring-1 ring-slate-900/10 shadow-lg overflow-hidden w-30 py-1 text-sm text-slate-700 font-semibold dark:bg-slate-800 dark:ring-0 dark:highlight-white/5 dark:text-slate-300 mt-4">
